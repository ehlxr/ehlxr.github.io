...
(function ($) {
	...
	var play = function (showNotice) {
		...
		if (promise !== undefined) {
			promise.then(_ => {
				...
				// showTips('#player .ctrl .progress .contr .playback', '已播放音乐！点击暂停。');
			}).catch(error => {
				console.warn("浏览器禁止自动播放音乐！"+error);
				// setTimeout("showTips('#player .cover','点击封面开启(关闭)随机播放', " + function () {
				// 	setTimeout("showTips('#player .ctrl .musicTag','点击拖动标题栏快进(快退)')", 1000)
				// } + ");", 500);

				showNotification("浏览器禁止自动播放音乐！");
				// showTips('#player .ctrl .progress .contr .playback', '浏览器禁止自动播放！点击播放');
			});
		}
	}
	...
	var loadMusic = function (i) {
		...
		var newaudio = $('<audio src="' + item.mp3 + '">').appendTo('#player');
	...
	$("#QPlayer .ssBtn").on('click', function () {
		...
		if ($('.ssBtn .adf').hasClass('on') === true) {
			...
			mA.addClass("QPlayer-shadow");
			$('.ssBtn .adf').removeClass('on');

			$('.ssBtn').css("float", "right")
			...
			mA.removeClass("QPlayer-shadow");
			$('.ssBtn .adf').addClass('on')

			$('.ssBtn').css("float", "left")
			$('#player').css("float", "left")
		}
	});
...
function showTips(div, info, func) {
	var box_height = 100;
	$('body').append('<div class="qplayer_tips"><span class="tips_arrow"></span><span class="info" style="display:none">' + info + '</span><button class="tips_button" onclick="removeTips()">关闭提示</button></div>');
	// $('.qplayer_tips').css({ "top": $(div).offset().top - box_height - 30 - 15, "right": 30 });
	$('.qplayer_tips').css({ "top": $(div).offset().top - box_height - 30 - 15 });
	...
	// $('.tips_arrow').css({"border-width": "15px", "transition": "all .5s ease-in-out", "left": document.body.clientWidth - $(div).offset().left + 40 });
